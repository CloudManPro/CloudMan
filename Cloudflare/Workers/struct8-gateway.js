...
// src/auth/verifyCognitoJwt.ts
var cachedJwks = null;
var cachedForPoolId = null;
function getJwks(userPoolId, region) {
  if (cachedJwks && cachedForPoolId === userPoolId)
    return cachedJwks;
  const jwksUrl = new URL(
    `https://cognito-idp.${region}.amazonaws.com/${userPoolId}/.well-known/jwks.json`
  );
  cachedJwks = createRemoteJWKSet(jwksUrl);
  cachedForPoolId = userPoolId;
  return cachedJwks;
}
...
async function verifyCognitoJwt(request, env2) {
  const token = extractBearerToken(request);
  if (!token) {
    throw new Error("Token ausente (nem Authorization nem Sec-WebSocket-Protocol).");
  }
  const jwks = getJwks(env2.COGNITO_USER_POOL_ID, env2.COGNITO_REGION);
  const expectedIssuer = `https://cognito-idp.${env2.COGNITO_REGION}.amazonaws.com/${env2.COGNITO_USER_POOL_ID}`;
  const { payload } = await jwtVerify(token, jwks, {
    issuer: expectedIssuer
  });
  if (payload.token_use !== "id") {
    throw new Error(`token_use inesperado: ${String(payload.token_use)}`);
  }
  if (payload.aud !== env2.COGNITO_CLIENT_ID) {
    throw new Error("Claim aud n\xE3o corresponde ao COGNITO_CLIENT_ID configurado.");
  }
  if (typeof payload.sub !== "string" || !payload.sub) {
    throw new Error("Claim sub ausente ou inv\xE1lido no token.");
  }
  ...
  return {
    sub: payload.sub,
    email: emailVerificado ? email : void 0,
    exp: typeof payload.exp === "number" ? payload.exp : void 0
  };
}
__name(verifyCognitoJwt, "verifyCognitoJwt");
...
var ROUTES = {
  "/v1/hcl/aws": (env2, payload) => env2.STRUCT8_HCLAWS.generate(payload)
};
var gateway_default = {
  async fetch(request, env2) {
    ...
    if (!route) {
      return withCors(Response.json({ error: "Rota desconhecida." }, { status: 404 }));
    }
    if (request.method !== "POST") {
      return withCors(Response.json({ error: "Use POST." }, { status: 405 }));
    }
    try {
      await verifyCognitoJwt(request, env2);
    } catch (err) {
      return withCors(
        Response.json(
          { error: "N\xE3o autenticado.", detail: err instanceof Error ? err.message : String(err) },
          { status: 401 }
        )
      );
    }
    ...
    try {
      ...
      return withCors(
        Response.json(
          { error: "Corpo n\xE3o \xE9 JSON v\xE1lido.", detail: String(err) },
          { status: 400 }
        )
      );
    }
    let result;
    try {
      result = await route(env2, payload);
    } catch (err) {
      return withCors(
        Response.json(
          { error: "Servi\xE7o indispon\xEDvel.", detail: err instanceof Error ? err.message : String(err) },
          { status: 502 }
        )
      );
    }
