...
essential_vars=(
    ...
    "WPDOMAIN"
    # MANAGEMENT_WPDOMAIN é opcional, tratado abaixo
)
error_found=0
...
mount_efs() {
    ...
    if mount | grep -q "on ${mount_point} type efs"; then
        ...
        if [ -n "$efs_ap_id" ]; then
            ...
            mount_source="$efs_id" # Sem ":/" para AP
...
# Função para criar um template wp-config.php
