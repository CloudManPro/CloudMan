...
def lambda_handler(event, context):
    ...
    active_instance_ids = [instance['InstanceId']
                           for instance in instances if instance['LifecycleState'] == 'InService']
    ...
    snapshot = ec2_client.create_snapshot(
        VolumeId=volume_id, Description=f"Backup antes de terminar {instance_id}")
    snapshot_id = snapshot['SnapshotId']
    print(
        f"Snapshot {snapshot_id} iniciado para o volume {volume_id} da instância {instance_id}")
    ...
    try:
        ...
        print(
            f"ASG '{ASGName}' atualizado com sucesso para MaxSize=0, MinSize=0, DesiredCapacity=0.")
    ...
    if active_instance_ids:
        ...
        print(
            f"Nenhuma instância ativa encontrada no ASG '{ASGName}' para terminar.")

    # ********** Passo 8: desanexar e deregistar a AMI
    amis = ec2_client.describe_images(
        Filters=[{'Name': 'tag:Name', 'Values': [ASGName]}])
